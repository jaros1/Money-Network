angular.module('MoneyNetwork')
    ...
    .factory('MoneyNetworkHubService', ['$timeout', 'safeApply', 'brFilter', '$sanitize', '$rootScope', function($timeout, safeApply, br, $sanitize, $rootScope) {
        ...
        function detected_client_log_out (pgm, userid) {
            ...
            //    console.log(pgm + 'stop. not logged in. no ZeroNet cert. stopping ' + pgm + ' process') ;
            //    return true ;
            //}
            if (typeof userid == 'number') {
                return (z_cache.user_id != userid) ;
            }
            else if (z_cache.user_id) return false ;
                ...
                if (res == 'ok') {
                    ...
                    z_file_get(pgm, {inner_path: inner_path}, function(res, extra) {
                        ...
                        if (res) {
                            ...
                            console.log(pgm + 'mergerSiteAdd failed. Hub ' + hub + ' was not added. extra = ' + JSON.stringify(extra)) ;
                            msg = [ 'Could not add user data hub ' + hub, 'Maybe user data hub without peers', 'Maybe running on a proxy server with add site disabled'] ;
                            if (z_cache.my_user_hub == hub) {
                                msg.push('User profile could not be created') ;
                                msg.push('User action required (todo)')
                            }
                            console.log(pgm + msg.join('. ')) ;
                            z_wrapper_notification(['error', msg.join('<br>')]) ;

                            if (z_cache.my_user_hub == hub) {
                                // get a new list of user hubs. Maybe add fallback to an other user data hub
        ...
        function get_default_user_hub () {
            ...
            default_user_hub = {hub: '1PgyTnnACGd1XRdpfiDihgKwYRRnzgz2zh', title: 'U3 User data hub'} ;
        ...
        function get_my_user_hub (cb) {
            ...
            var step_1_get_mn_user_data_hubs, step_2_compare_json_and_files, step_3_find_user_hub, step_4_get_and_add_default_user_hub,
                step_5_user_hub_selected, step_6_run_callbacks, old_userid, user_data_hubs ;
            ;
            ...
            if (!z_cache.user_id) {
                console.log(pgm + "Not logged in. hub = 'n/a'") ;
                return cb('n/a','n/a', 'n/a') ;
            }
            ...
            if (z_cache.my_user_hub) return cb(z_cache.my_user_hub, z_cache.other_user_hub, z_cache.other_user_hub_title) ;
            ...
            step_6_run_callbacks = function () {
                ...
                cb(z_cache.my_user_hub, z_cache.other_user_hub, z_cache.other_user_hub_title) ;
            ...
            step_5_user_hub_selected = function () {
                ...
                if (user_data_hubs.length <= 1) {
                    ...
                    delete z_cache.other_user_hub_title ;
                    return step_6_run_callbacks() ;
                }
                other_user_data_hubs = [] ;
                for (i=0 ; i<user_data_hubs.length ; i++) {
                    if (user_data_hubs[i].hub != z_cache.my_user_hub) other_user_data_hubs.push(user_data_hubs[i]) ;
                }
                if (!other_user_data_hubs.length) {
                    // debug info. trying to fix a JS error
                    console.log(pgm + 'error. expected ' + (user_data_hubs.length-1) + ' other_user_data_hubs') ;
                    console.log(pgm + 'my_user_hub = ' + z_cache.my_user_hub) ;
                ...
                if (!other_user_data_hubs[i]) {
                    // debug info. trying to fix a JS error
                    console.log(pgm + 'error. expected i between 0 and ' + ( other_user_data_hubs.length-1)) ;
                    console.log(pgm + 'i = ' + i + ', other_user_data_hubs.length = ' + other_user_data_hubs.length) ;
                    // fix system error
                    z_cache.other_user_hub = z_cache.my_user_hub ;
                    delete z_cache.other_user_hub_title ;
                    return step_6_run_callbacks() ;
                }
                z_cache.other_user_hub = other_user_data_hubs[i].hub ; // = data.json hub
                z_cache.other_user_hub_title = other_user_data_hubs[i].title ; // = data.json hub_title
                return step_6_run_callbacks() ;
            }; // step_5_user_hub_selected

            // normally never used. default_hubs should be in user_data_hubs array
            step_4_get_and_add_default_user_hub = function () {
                ...
                z_merger_site_add(my_user_hub.hub, function (res) {
                    ...
                    if (res == 'ok') {
                        z_cache.my_user_hub = my_user_hub.hub ;
                        user_data_hubs.push(my_user_hub) ;
                        step_5_user_hub_selected() ;
                        return ;
                    }
                    console.log(pgm + 'mergerSiteAdd failed. hub = ' + my_user_hub + '. error = ' + res) ;

                }) ; // z_merger_site_add callback

            }; // step_4_get_and_add_default_user_hub

            step_3_find_user_hub = function () {
                var pgm = service + '.get_my_user_hub.step_3_find_user_hub: ' ;
                var mn_query_17, i, debug_seq ;

                if (!user_data_hubs.length) {
                    console.log(pgm + 'No user data hubs were found. user_data_hubs.length = 1. Must be first contact.') ;
                ...
                for (i=0 ; i<user_data_hubs.length ; i++) {
                    mn_query_17 += (i == 0 ? "('" : ",'") + user_data_hubs[i].hub + '/data/users/' + ZeroFrame.site_info.auth_address + "'" ;
                }
                mn_query_17 += ") " +
                ...
                debug('select', pgm + 'mn query 17 = ' + mn_query_17);
                ...
                ZeroFrame.cmd("dbQuery", [mn_query_17], function (res) {
                    var pgm = service + '.get_my_user_hub.step_3_find_user_hub dbQuery callback: ' ;
                    var i, merge_job, priority_texts , priorities, priority, min_priority ;
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    ...
                    if (res.error) {
                        ...
                        console.log(pgm + 'query = ' + mn_query_17);
                        res.length = 0 ;
                    }
                    if (res.length) {
                    ...
                    if (!user_data_hubs.length) {
                        console.log(pgm + 'error. user_data_hubs is empty. default_hubs should be in user_data_hubs') ;
                        return step_4_get_and_add_default_user_hub() ;
                    }
            ...
            }; // step_3_find_user_hub

            // compare "json" and "files" tables before running query 17 in step_3_find_user_hub. Should be identical. files table is no longer used in query 17
            step_2_compare_json_and_files = function () {
                var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files: ' ;
                var mn_query_6, debug_seq1 ;
                ...
                debug('select', pgm + 'mn query 6 = ' + mn_query_6) ;
                debug_seq1 = debug_z_api_operation_start(pgm, 'mn query 6', 'dbQuery', show_debug('z_db_query')) ;
                ZeroFrame.cmd("dbQuery", [mn_query_6], function (res1) {
                    var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files dbQuery callback 1: ';
                    var mn_query_19, debug_seq2, res, i, index, indexed_by_modified ;
                    debug_z_api_operation_end(debug_seq1, (!res1 || res1.error) ? 'Failed. error = ' + JSON.stringify(res1) : 'OK');
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    if (res1.error) {
                        console.log(pgm + "json and files compare failed: " + res1.error);
                        console.log(pgm + 'mn query 8 = ' + mn_query_6);
                        return step_3_find_user_hub() ;
                    }
                    res = {} ;
                    indexed_by_modified = {} ;
                    ...
                    debug('select', pgm + 'mn query 19 = ' + mn_query_19) ;
                    debug_seq2 = debug_z_api_operation_start(pgm, 'mn query 19', 'dbQuery', show_debug('z_db_query')) ;
                    ZeroFrame.cmd("dbQuery", [mn_query_19], function (res2) {
                        var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files dbQuery callback 2: ';
                        var i, index, hub, missing_rows, dictionaries, modified, sign, user_data_hub ;
                        ...
                        if (res2.error) {
                            ...
                            console.log(pgm + 'mn query 19 = ' + mn_query_19);
                            return step_3_find_user_hub();
                        }
                        ...
                        for (index in res) {
                            ...
                            user_data_hub = false ;
                            for (i=0 ; i<user_data_hubs.length ; i++) {
                                if ((user_data_hubs[i].hub == hub) && !user_data_hubs[i].hub_added_at) {
                                    user_data_hub = true ;
                                    break ;
                                }
                            }
                            if (!user_data_hub) continue ; // ignore - not a MN User Data Hub - also ignore just added User Data Hubs
                            missing_rows.push(res[index]) ;
                        }
                        if (!missing_rows.length) return step_3_find_user_hub() ; // everything is OK. next step
                        ...
                        //        "in_files": true
                        //    },
                        //    "1PgyTnnACGd1XRdpfiDihgKwYRRnzgz2zh/data/users/18DbeZgtVCcLghmtzvg4Uv8uRQAwR8wnDQ/avatar.jpg": {
                        //        "directory": "1PgyTnnACGd1XRdpfiDihgKwYRRnzgz2zh/data/users/18DbeZgtVCcLghmtzvg4Uv8uRQAwR8wnDQ",
                        ...
                        sign = function () {
                            var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files.sign: ';
                            ...
                            z_file_get(pgm, {inner_path: inner_path, required: true}, function (content_str) {
                                var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files.sign z_file_get callback 1: ';
                                ...
                                z_file_write(pgm, inner_path, btoa(json_raw), function (res) {
                                    var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files.sign fileWrite callback 2: ';
                                    var debug_seq2 ;
                                    if (detected_client_log_out(pgm, old_userid)) return ;
                                    if (res != 'ok') {
                                        console.log(pgm + 'Error: ' + inner_path + ' fileWrite failed. res = ' + JSON.stringify(res));
                                        return step_3_find_user_hub() ; // error - continue with next step
                                    }
                                    // 3: sign
                                    ...
                                    ZeroFrame.cmd("siteSign", {inner_path: inner_path, remove_missing_optional: true}, function (res) {
                                        var pgm = service + '.get_my_user_hub.step_2_compare_json_and_files.sign siteSign callback 3: ';
                                        ...
                                        // sign ok
                                        if (!dictionaries.length) return step_3_find_user_hub() ; // done - continue with next step
            ...
            step_1_get_mn_user_data_hubs = function () {
                var pgm = service + '.get_my_user_hub.step_1_get_mn_user_data_hubs: ';
                MoneyNetworkAPILib.get_all_hubs(false, function (all_hubs)  {
                    var i ;
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    // filter relevant hubs.
                    for (i=0 ; i<all_hubs.length ; i++) {
                        if (all_hubs[i].hub_type == 'user') user_data_hubs.push(all_hubs[i]);
                    }
                    console.log(pgm + 'user_data_hubs = ' + JSON.stringify(user_data_hubs));
                    // next step
                    step_2_compare_json_and_files() ;
                }) ;
            } ; // step_1_get_mn_user_data_hubs
        ...
        function merge_user_hub (hub, cb) {
            ...
            get_my_user_hub(function(my_user_data_hub, other_user_hub, other_user_hub_title) {
                ...
                hub_user_path = 'merged-' + get_merged_type() + '/' + hub + '/data/users/' + ZeroFrame.site_info.auth_address + '/' ;
                my_user_path = 'merged-' + get_merged_type() + '/' + my_user_data_hub + '/data/users/' + ZeroFrame.site_info.auth_address + '/' ;
                ...
                z_file_get(pgm, {inner_path: inner_path0, required: false}, function (hub_content_str) {
                    var pgm = service + '.merge_user_hub z_file_get callback 2: ';
                    var hub_content, my_content, hub_data, my_data, hub_status, my_status,  my_like, hub_like,
                        ...
                        step_13_write_and_publish_user_data, step_n_done, json_raw, status ;
                    ...
                    // for example "This site requests permission: Merger:MoneyNetwork" or dbQuery error "'NoneType' object has no attribute 'execute'"
                    var step_1_update_content_json = function (cb1) {
                        ...
                        // # https://github.com/HelloZeroNet/ZeroNet/issues/1147 & https://github.com/jaros1/Money-Network/issues/306
                        ...
                        ZeroFrame.cmd("fileWrite", [inner_path0, btoa(json_raw)], function (res) {
                            ...
                            ZeroFrame.cmd("siteSign", {inner_path: inner_path0, remove_missing_optional: true}, function (res) {
                                ...
                                z_file_get(pgm, {inner_path: inner_path0, required: false}, function (new_hub_content_str) {
                    ...
                    step_2_remove_file_doublets = function (cb2) {
                        ...
                        z_file_get(pgm, {inner_path: inner_path0, required: false}, function (content_str) {
                            var pgm = service + '.merge_user_hub.step_2_remove_file_doublets z_file_get callback: ';
                    ...
                    step_5_get_data_files = function (cb5) {
                        ...
                        get_data_json(function (data) {
                            ...
                            z_file_get(pgm, {inner_path: inner_path0, required: true}, function (data_str) {
                                var pgm = service + '.merge_user_hub.step_5_get_data_files z_file_get callback 2: ';
                                var error ;
                    ...
                    step_6_get_status_files = function (cb6) {
                        ...
                        get_status_json(function (status) {
                            ...
                            z_file_get(pgm, {inner_path: inner_path0, required: true}, function (status_str) {
                                var pgm = service + '.merge_user_hub.step_6_get_status_files z_file_get callback 2: ';
                                var error ;
                    ...
                    step_7_get_like_files = function (cb7) {
                        ...
                        get_like_json(function (like) {
                            ...
                            z_file_get(pgm, {inner_path: inner_path0, required: true}, function (like_str) {
                                var pgm = service + '.merge_user_hub.step_7_get_like_files z_file_get callback 2: ';
                                var error ;
                    ...
                    step_9_merge_user = function (cb9) {
                        ...
                        console.log(pgm + 'hub_user = ' + JSON.stringify(hub_user) + ', my_user = ' + JSON.stringify(my_user)) ;
                        ...
                        if (!my_user) {
                            ...
                            status.my_data_updated = true ;
                        }
                        my_user_seq = my_user.user_seq ;
                        hub_user_seq = hub_user.user_seq ;
                        // data.json search array todo: check. search array is not copied to current user hub
                        ...
                        move_user_files = function() {
                            ...
                            for (key in hub_content.files_optional) {
                                ...
                                if (!hub_content.files_optional[key].file_info.is_downloaded &&
                                    !hub_content.files_optional[key].file_info.peer) continue ; // not downloaded and no peers
                                if (hub_content.files_optional[key].file_info.is_downloaded == -1) continue ; // download failed
                                if (!hub_content.files_optional[key].file_str) continue ; // z_file_get failed
                    ...
                    step_11_cleanup_hub_user_files = function(cb11) {
                        ...
                        if (!status.hub_publish) {
                            console.log(pgm + 'no hub user files were updated') ;
                            return cb11() ;
                        }
                        if (!hub_content.files) {
                            console.log(pgm + 'no hub user files') ;
                            return cb11() ;
                        }
                        if (Object.keys(hub_content.files).length == 0) {
                            console.log(pgm + 'no more hub user files - next step') ;
                            return cb11() ;
                        }
                    ...
                    step_12_publish_empty_hub = function (cb12) {
                        ...
                        if (!status.hub_publish) return cb12() ;

                        // write hub content without any optional files to prevent missing optional files error
                        // https://github.com/HelloZeroNet/ZeroNet/issues/1147 & HelloZeroNet/ZeroNet#1147
                        // delete hub_content.files_optional ;
                        ...
                        z_file_write(pgm, inner_path , btoa(json_raw), function (res) {
                            ...
                            zeronet_site_publish({inner_path: inner_path, reason: 'merge_user_hub'}, function (res) {
                    ...
                    step_13_write_and_publish_user_data = function (cb13) {
                        ...
                        write_data = function (cb) {
                            ...
                            var json_raw, inner_path, debug_seq ;
                            if (!status.my_data_updated) return cb() ;
                            status.my_publish = true ;
                            json_raw = unescape(encodeURIComponent(JSON.stringify(my_data, null, "\t")));
                            inner_path = my_user_path + 'data.json' ;
                        ...
                        write_status = function (cb) {
                            ...
                            var json_raw, inner_path, debug_seq ;
                            if (!status.my_status_updated) return cb() ;
                            status.my_publish = true ;
                        ...
                        write_like = function (cb) {
                            ...
                            var json_raw, inner_path, debug_seq ;
                            if (!status.my_like_updated) return cb() ;
                            status.my_publish = true ;
                            json_raw = unescape(encodeURIComponent(JSON.stringify(my_like, null, "\t")));
                            inner_path = my_user_path + 'like.json' ;
                            // debug_seq = MoneyNetworkHelper.debug_z_api_operation_start('z_file_write', pgm + inner_path + ' fileWrite') ;
                            debug_seq = debug_z_api_operation_start(pgm, inner_path, 'fileWrite', show_debug('z_file_write')) ;
                            z_file_write(pgm, inner_path , btoa(json_raw), function (res) {
                            // ZeroFrame.cmd("fileWrite", [inner_path , btoa(json_raw)], function (res) {
                        ...
                        publish = function (cb) {
                            ...
                            var inner_path0, debug_seq0;
                            if (!status.my_publish) return cb();

                            inner_path0 = my_user_path + 'content.json';
                            // MoneyNetworkAPILib.z_site_publish({inner_path: inner_path0, reason: 'merge_user_hub'}, function (res) {
                            zeronet_site_publish({inner_path: inner_path0, reason: 'merge_user_hub'}, function (res) {
        ...
        function get_data_json (cb) {
            ...
            get_my_user_hub (function (my_user_data_hub, other_user_hub, other_user_hub_title) {
                ...
                z_file_get(pgm, {inner_path: inner_path, required: false, timeout: 1}, function (data_str) {
                    var pgm = service + '.get_data_json z_file_get callback 1: ';
                    var data, empty;
                    console.log(pgm + inner_path + ' fileGet finished') ;
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    if (data_str) {
                        try {
        ...
        function write_data_json (cb) {
            ...
            console.log(pgm + 'calling get_my_user_hub');
            get_my_user_hub (function (my_user_data_hub, other_user_hub, other_user_hub_title) {
                var pgm = service + '.write_data_json get_my_user_hub callback 1: ' ;
                ...
                z_file_write(pgm, user_path + '/data.json', btoa(json_raw), function (res) {
                    var pgm = service + '.write_data_json z_file_write callback 2: ' ;
                    var debug_seq2 ;
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    if (res == 'ok') {
                        // content.json must always exists
                        debug_seq2 = debug_z_api_operation_start(pgm, user_path + '/content.json', 'siteSign', show_debug('z_site_publish')) ;
                        ZeroFrame.cmd("siteSign", {inner_path: user_path + '/content.json', remove_missing_optional: true}, function (res2) {
                            var pgm = service + '.write_data_json siteSign callback 3: ' ;
        ...
        function get_status_json (cb) {
            ...
            get_my_user_hub(function (my_user_data_hub, other_user_hub, other_user_hub_title) {
                ...
                // read status.json into cache
                z_file_get(pgm, {inner_path: inner_path, required: false, timeout: 1}, function (status) {
        ...
        function get_like_json(cb) {
            ...
            if (!ZeroFrame.site_info || ZeroFrame.site_info.cert_user_id || !z_cache.user_id) {
                ...
                return cb(z_cache.like_json, z_cache.like_json_index, true);
            }
            if (detected_client_log_out(pgm, old_userid)) return;
            ...
            get_user_seq(function (my_user_seq) {
                ...
                get_my_user_hub(function (my_user_data_hub, other_user_hub, other_user_hub_title) {
                    ...
                    z_file_get(pgm, {inner_path: inner_path, required: false, timeout: 1}, function (like_str) {
        ...
        function write_like_json (cb) {
            ...
            var user_path, like, json_raw, old_userid ;
            old_userid = z_cache.user_id ;
            if (detected_client_log_out(pgm, old_userid)) return ;
            ...
            get_my_user_hub(function (my_user_data_hub, other_user_hub, other_user_hub_title) {
                ...
                z_file_write(pgm, user_path + '/like.json', btoa(json_raw), function (res) {
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    cb(res) ;
                }) ; // fileWrite callback 2
            }) ; // get_my_user_hub callback 1
                        ...
                        write_status_json(function (res) {
                            ...
                            MoneyNetworkAPILib.z_site_publish({inner_path: user_path + '/content.json', remove_missing_optional: true, reason: reason}, function (res) {
                                ...
                                if (res != "ok") {
                                    ...
                                    MoneyNetworkAPILib.get_all_hubs(true, function (all_hubs) {
                                        ...
                                        if (peers) {
                                            ...
                                            // todo: no reason to start retry publish loop if problem is no peers
                                            ...
                                            if (!zeronet_site_publish_no_peers && new_user_data_hub) {
                                                ...
                                                ZeroFrame.cmd("wrapperConfirm", [msg.join('<br>'), 'OK'], function (confirm) {
                                                    ...
                                                    move_user_hub(new_user_data_hub, function (res) {
                                ...
                                z_file_get(pgm, {inner_path: user_path + '/content.json', required: false}, function (content) {
                                    ...
                                    z_file_write(pgm, user_path + '/content.json', btoa(json_raw), function (res2) {
        ...
        function check_sha256_addresses (context, update_local_storage, correct_errors) {
            ...
            get_data_json(function (data) {
                var pgm = service + '.check_sha256_addresses get_data_json callback for ' + context + ': ';
                ...
                my_pubkey = z_cache.user_id ? MoneyNetworkHelper.getItem('pubkey') : 'n/a';
            ...
            }) ; // end get_data_json callback (check data.json)
        ...
        function get_user_seq (cb) {
            ...
            var old_userid ;
            old_userid = z_cache.user_id ;
            if (!z_cache.user_id) return cb(0) ; // not logged in. Show public chat only
            if (detected_client_log_out(pgm, old_userid)) return ;
            // check z_cache
            if (z_cache.user_seq) {
            ...
            get_data_json(function (data) {
                var pgm = service + '.get_user_seq get_data_json callback 1: ';
        ...
        // request Merger:MoneyNetwork permission
        // not much going on in there without merger permissions
        // also check number of hubs. there should always be an user data hub
        function request_merger_permission (pgm, cb) {
            ZeroFrame.cmd("wrapperPermissionAdd", "Merger:MoneyNetwork", function (res) {
                if (res != 'Granted') {
                    console.log(pgm + 'error. wrapperPermissionAdd failed. res = ' + JSON.stringify(res)) ;
                    return cb(res) ;
                }
                // refresh list of hubs. no added user data hubs at this time
                MoneyNetworkAPILib.get_all_hubs(true, function(all_hubs) {
                    var missing_hubs, i, hub ;
                    console.log(pgm + 'all_hubs = ' + JSON.stringify(all_hubs)) ;
                    missing_hubs = [] ;
                    for (i=0 ; i<all_hubs.length ; i++) {
                        if (all_hubs[i].hub_added) return cb('ok') ;
                        else missing_hubs.push(all_hubs[[i]]) ;
                    }
                    i = Math.floor(Math.random() * missing_hubs.length) ;
                    hub = missing_hubs[i].hub ;
                    console.log(pgm + 'adding user data hub ' + hub) ;
                    z_merger_site_add(hub, function (res) {
                        console.log(pgm + 'res = ' + JSON.stringify(res)) ;
                        return cb('ok') ;
                    }) ; // z_merger_site_add
                }) ;
        ...
        function check_merger_permission (calling_pgm, cb) {
            ...
            ZeroFrame.cmd("siteInfo", {}, function (site_info) {
                ...
                // check user data hubs. there should always be an user data hub
                MoneyNetworkAPILib.get_all_hubs(false, function (all_hubs) {
                    var pgm = service + '.check_merger_permission get_all_hubs callback 2: ';
                    var i, missing_hubs, hub ;
                    missing_hubs = [] ;
                    for (i=0 ; i<all_hubs.length ; i++) {
                        if (all_hubs[i].hub_type != 'user') continue ;
                        if (all_hubs[i].hub_added) {
                            // OK. found
                            console.log(pgm + 'found already added user data hub ' + all_hubs[i].hub) ;
                            return cb('ok') ;
                        }
                        missing_hubs.push(all_hubs[i]) ;
                    }
                    if (!missing_hubs.length) return cb() ;
                    i = Math.floor(Math.random() * missing_hubs.length) ;
                    hub = missing_hubs[i].hub ;
                    console.log(pgm + 'adding user data hub ' + hub) ;
                    z_merger_site_add(hub, function (res) {
        ...
        function write_empty_chat_file (file_path, cb) {
            ...
            debug_seq = debug_z_api_operation_start(pgm, file_path, 'fileWrite', show_debug('z_file_write')) ;
            z_file_write(pgm, file_path, btoa(json_raw), function (res) {
        ...
        function get_my_unique_id () {
            ...
            if (!ZeroFrame.site_info.cert_user_id) return null ; // ZeroNet certificate not selected
            if (!z_cache.user_id) return null ; // not logged in
        ...
        function i_am_online () {
            ...
            get_my_user_hub(function (my_user_hub, other_user_hub, other_user_hub_title) {
                ...
                user_path = "merged-" + get_merged_type() + "/" + my_user_hub + "/data/users/" + ZeroFrame.site_info.auth_address;
                ...
                console.log(
                    ...
                    ' and my user data hub is ' + my_user_hub) ;
                debug('site_info', pgm + 'site_info = ' + JSON.stringify(ZeroFrame.site_info));
                console.log(pgm + 'other_user_hub = ' + other_user_hub + ', other_user_hub_title = ' + other_user_hub_title) ;

                // prevent user from removing user data hub with user data.
                MoneyNetworkAPILib.get_all_hubs(true, function (all_hubs) {}) ;
                set_disabled_job = function () { MoneyNetworkAPILib.all_hubs_set_disabled(my_user_hub) } ;
                ...
                get_data_json(function (data, empty) {
                    ...
                    var my_user_seq, data_user_seqs, i, pubkey, pubkey2, ls, compare, key, count, in_both,
                        check_pubkey2_cb, debug_seq, inner_path ;
                    if (detected_client_log_out(pgm, old_userid)) return ;
                    pubkey = MoneyNetworkHelper.getItem('pubkey') ;
                    ...
                    check_pubkey2_cb = function () {
                        ...
                        debug_seq = debug_z_api_operation_start(pgm, null, 'userPublickey', show_debug('z_crypt_message')) ;
                        ZeroFrame.cmd("userPublickey", [rec.l_userid], function (pubkey2) {
                            // MoneyNetworkHelper.debug_z_api_operation_end(debug_seq) ;
                            debug_z_api_operation_end(debug_seq, pubkey2 ? 'OK' : 'Failed. No pubkey returned') ;
        ...
        return {
            detected_client_log_out:  detected_client_log_out,
